...
public class Quiz_7 {  
	public static void main(String[] args) {
		...
		while(flag) {
			...
			System.out.print("결과 : ");
			//same 배열을 불러오기 위한 임시 변수
			int tmp = 0;	
			//결과 문자열을 출력한다.
			for (int i = 0; i < charArray.length; i++) {
				//찾는 문자열 길이가 1 일때 원래 문장 출력을 제한 하기 위한 변수
				int condition = 0;
				//조건을 만족하면 바꿀 문자열을 출력하고 아니면 원래 문자열을 출력한다. 찾는 문자열 길이가 1 일 때
				if (searchArray.length == 1 && i > sameNums[tmp] - searchArray.length && i <= sameNums[tmp]) {
					...
					tmp++;
					//원래 문장을 출력하지 않기 위해 증가 
					condition++;
					//조건을 만족하면 바꿀 문자열을 출력하고 아니면 원래 문자열을 출력한다. 찾는 문자열 길이가 1 보다 클 때 
				...
				//찾는 문자열 길이가 1 일 때 출력하지 않기 위한 조건
				if (condition == 0) {
					System.out.print(charArray[i]);
				}
			}
